...
- Tests use real time (`Thread.sleep`) to validate TTL. If tests fail due to timing on a slow machine, increase TTL values in tests slightly.  
- The cleaner thread is a daemon thread; `shutdown()` is called in tests to avoid leftover threads.  
- Edge case to mention in interview: multiple `put` calls for the same key create multiple `DelayedCacheItem` entries in the `DelayQueue`. A robust improvement is to store expiration metadata with the value and have the cleaner verify the stored expiration before removing the key.

---

### Improvements and extensions
- Add **max size** and LRU eviction.  
- Replace `String` values with generic JSON objects using Jackson and `Object` or typed DTOs.  
- Expose metrics (hit/miss counts) and monitoring endpoints.  
- Make the cache a Spring-managed bean and add configuration properties for defaults.  
- For distributed caching, integrate Redis or Hazelcast.
